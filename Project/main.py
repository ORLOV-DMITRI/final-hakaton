...
class Main(Resource):
    def get(self, city):
        with sq.connect('City.sqlite3') as con:
            ...
            kz_data = get_list_db(cur.fetchall())
            cur.execute('SELECT ru_name FROM City_name')
            ru_data = get_list_db(cur.fetchall())
        if city in kz_data:
            index = kz_data.index(city)
            city = ru_data[index]
...
class WorkDataBase(Resource):
    ...
    def post(self, text):
        ...
        if text == 'x': # Проверка пустая ли строка, если да, то просто возвращает все данные
            with sq.connect('City.sqlite3') as con:
                cur = con.cursor()
    ...
    def delete(self, text):
        ...
        with sq.connect('City.sqlite3') as con:
            ...
            cur.execute(f'''DELETE FROM City_save
...
class KzData(Resource):
    def post(self, text):
        ...
        with sq.connect('City.sqlite3') as con:
            ...
            if len(db_text) <= 2:
                cur.execute(
                    f"""INSERT INTO City_save (Name) VALUES ('{name_city_ru}')""")
                con.commit()
            elif len(db_text) > 2:
                cur.execute(
                    f"DELETE FROM City_save WHERE rowid = '{rowid[0]}';")
                con.commit()
                cur.execute(
                    f"""INSERT INTO City_save (Name) VALUES ('{name_city_ru}')""")
                con.commit()
